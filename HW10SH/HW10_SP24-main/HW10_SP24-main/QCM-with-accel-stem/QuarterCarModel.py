...
class CarModel():
    ...
    def __init__(self):
        ...
        self.m2 = 10  # mass of wheel in kg
        self.c1 = 10  # damping coefficient in N*s/m
        self.k1 = 5000  # spring constant of suspension in N/m
        self.k2 = 500  # spring constant of tire in N/m
        self.v = 100  # velocity of car in kph


        self.mink1 = 450 * 9.81 * 0.75 / 5000  #If I jack up my car and release the load on the spring, it extends about 3 inches
        self.maxk1 = 450 * 9.81 * 3 / 5000  #What would be a good value for a soft spring vs. a stiff spring?
        self.mink2 = 450 * 9.81 * 0.75 / 500  #Same question for the shock absorber.
        self.maxk2 = 450 * 9.81 * 3 / 500
        self.accel =None
        self.accelMax = 2 * 9.81
        self.accelLim = 2 * 9.81
        self.SSE = 0.0
...
class CarController():
    ...
    def ode_system(self, X, t):
        ...
        x1ddot = (self.model.k1 * (x2 - x1) - self.model.c1 * (x2dot - x1dot) - self.model.m1 * 9.81) / self.model.m1
        x2ddot = (self.model.k2 * (y - x2) - self.model.k1 * (x2 - x1) - self.model.c1 * (
                    x2dot - x1dot)) / self.model.m2
    ...
    def calculate(self, doCalc=True):
        ...
        self.mink1 = 0  # Assuming these are default values
        self.maxk1 = 1000  # Assuming these are default values
        self.mink2 = 0  # Assuming these are default values
        self.maxk2 = 1000  # Assuming these are default values
    ...
    def OptimizeSuspension(self):
        ...
        """
        #Step 1:
        self.setWidgets(None)
        self.calculate(doCalc=False)
        #Step 2:
        initial_guess = np.array([100, 10, 100])

        #Step 3:
        answer = minimize(self.SSE, initial_guess, method='Nelder-Mead')
        self.view.updateView(self.model)
